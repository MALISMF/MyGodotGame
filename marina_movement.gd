...
func handle_flying():
	var is_flying_pressed = Input.is_action_just_pressed("fly")
...
func handle_movement(delta):
	...
	if flying:
		player.velocity = direction * flying_speed
	else: 
...
func handle_animations():
	if was_moving:
		...
		match last_direction:
			"right-down":
				animated_sprite.play("side-front-walking" if !flying else "flying")
				animated_sprite.flip_h = false
			"left-down":
				animated_sprite.play("side-front-walking" if !flying else "flying")
				animated_sprite.flip_h = true
			"right-up":
				animated_sprite.play("side-back-walking" if !flying else "flying")
				animated_sprite.flip_h = false
			"left-up":
				animated_sprite.play("side-back-walking" if !flying else "flying")
				animated_sprite.flip_h = true
			"left":
				animated_sprite.play("side-walking" if !flying else "flying")
				animated_sprite.flip_h = true
			"right":
				animated_sprite.play("side-walking" if !flying else "flying")
				animated_sprite.flip_h = false
			"down":
				animated_sprite.play("walking" if !flying else "flying")
				animated_sprite.flip_h = false
			"up":
				animated_sprite.play("back-walking" if !flying else "flying")
				animated_sprite.flip_h = false
	else:
		# Стояние на месте - используем last_standing_direction
		match last_standing_direction:
			"right":
				animated_sprite.play("side-standing" if !flying else "flying")
				animated_sprite.flip_h = false
			"left":
				animated_sprite.play("side-standing" if !flying else "flying")
				animated_sprite.flip_h = true
			"right-up":
				animated_sprite.play("side-back-standing" if !flying else "flying")
				animated_sprite.flip_h = false
			"left-up":
				animated_sprite.play("side-back-standing" if !flying else "flying")
				animated_sprite.flip_h = true
			"left-down":
				animated_sprite.play("side-front-standing" if !flying else "flying")
				animated_sprite.flip_h = true
			"right-down":
				animated_sprite.play("side-front-standing" if !flying else "flying")
				animated_sprite.flip_h = false
			"up":
				animated_sprite.play("back-standing" if !flying else "flying")
				animated_sprite.flip_h = false
			"down":
				animated_sprite.play("standing" if !flying else "flying")
				animated_sprite.flip_h = false
